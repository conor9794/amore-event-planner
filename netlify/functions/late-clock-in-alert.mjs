...
function requiredSmsConfig() {
  ...
  if (missing.length) {
    throw new Error(`Melinda's late-alert SMS is not configured: ${missing.join(", ")}`);
  }
  return config;
}
...
async function sendSms(message) {
  ...
  if (!response.ok) {
    throw new Error(`Twilio rejected Melinda's late alert with status ${response.status}`);
  }
}
  ...
  for (const booking of due) {
    try {
      ...
      failures.push({
        ...
        error: error.message || "Melinda's late alert failed"
      });
    }
  }
