...
const {
  ...
  shouldSendLateAlert,
  lateAlertMessage
} = require("../netlify/functions/_late-alert-core.js");
...
test("alert remains eligible before start but never sends at or after start", () => {
  const base = {
    bookingConfirmed: true,
    scheduledStart: "2026-09-12T18:00:00Z"
  };

  assert.equal(shouldSendLateAlert({ ...base, now: "2026-09-12T17:59:59Z" }), true);
  assert.equal(shouldSendLateAlert({ ...base, now: "2026-09-12T18:00:00Z" }), false);
  assert.equal(shouldSendLateAlert({ ...base, now: "2026-09-12T18:10:00Z" }), false);
});
